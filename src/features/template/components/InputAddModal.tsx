...
import { AddDropdownValueToTemplate, AddInputValueToTemplate, AddQuantityFieldToTemplate, AddQuantityValueToTemplate } from "../store/TemplateSlice";
...
function InputAddModal({ sectionType, isOpen, rowIndex, columnIndex, inputIndex,inputType, onOpenChange, addInputToTemplate }: any) {
    ...
    const getOptionLabel = (option:any) => option?.name || option?.label || option?.value || option?.id || 'Option';
    ...
    const handleSave = () => {
        addInputToTemplate({input:selectedInput,count,show_quantity:showQuantity,quantity:selectedQuantityId});
        onOpenChange(false);
    }
                        ...
                        <div className="space-y-2">
                            ...
                            >
                                ...
                                {quantityOptions.map((option:any) => (
                                    <option key={option.id || option.value} value={option.id || option.value}>
                                        {option?.value || option?.label || option?.name || option?.id}
                                    </option>
                                ))}
                            </select>
                        </div>

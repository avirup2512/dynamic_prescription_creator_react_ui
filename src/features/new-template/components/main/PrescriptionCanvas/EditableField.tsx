...
export default function EditableField({ input, mode, selected, onSelect, onSettings, onQuickStyle, sectionId, rowId, columnId }: EditableFieldProps) {
    const [value, setValue] = useState(input.value ?? "");
    const label = input.showLabel === false ? "" : input.label;
        ...
        >
            {mode === "edit" && (
                <div className="absolute right-1.5 top-1.5 hidden items-center gap-1 group-hover/field:flex">
                    <button
                        type="button"
                        aria-label={`Quick style for ${input.label}`}
                        onClick={(event) => {
                    ...
                    <button
                        type="button"
                        aria-label={`Settings for ${input.label}`}
                        onClick={(event) => {
                            event.stopPropagation();
                            onSettings?.();
                        }}
            ...
            {label && <p className="mb-1 text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400">{label}</p>}

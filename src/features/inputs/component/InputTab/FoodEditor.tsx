...
const FoodEditor = memo(() => {
  ...
  const [selectedIds, setSelectedIds] = useState<string[]>(["steel-cut-oatmeal", "oat-bread"]);
  const [multiSelect, setMultiSelect] = useState(true);

  const categories = useMemo(() => ["All categories", ...Array.from(new Set(foodCatalog.map((food) => food.category)))], []);
  const filteredFoods = useMemo(
    () =>
      foodCatalog.filter((food) => {
        const matchesQuery = food.name.toLowerCase().includes(query.toLowerCase());
        const matchesCategory = category === "All categories" || food.category === category;
        return matchesQuery && matchesCategory;
      }),
    [category, query],
  );

  const selectedFoods = useMemo(() => foodCatalog.filter((food) => selectedIds.includes(food.id)), [selectedIds]);
  const focusedFood = selectedFoods[0] ?? filteredFoods[0] ?? foodCatalog[0];

  const toggleFood = useCallback((id: string) => {
    setSelectedIds((current) => {
      if (!multiSelect) return current.includes(id) ? [] : [id];
  ...
  return (
    <div className="grid h-full min-h-0 grid-cols-[minmax(0,1fr)_320px] gap-3 overflow-hidden">
      <section className="min-h-0 overflow-hidden rounded-lg border border-slate-200 bg-white">
        <div className="border-b border-slate-100 p-3">
          <div className="grid grid-cols-[minmax(0,1fr)_220px] gap-3">
            <div className="relative">
              ...
              <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search foods" className="h-9 w-full rounded-md border-2 border-blue-500 pl-9 pr-3 text-[13px] outline-none ring-2 ring-blue-100" />
            </div>
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            {["All", "Grains", "Dairy", "Protein", "Vegetables", "Fruit", "Beverages", "Diabetic-friendly"].map((pill) => (
              <button key={pill} type="button" onClick={() => setCategory(pill === "All" ? "All categories" : pill)} className="rounded-md border border-slate-200 px-2.5 py-1 text-[11px] text-slate-600 hover:bg-slate-50 data-[active=true]:border-blue-600 data-[active=true]:bg-blue-600 data-[active=true]:text-white" data-active={category === pill || (pill === "All" && category === "All categories")}>
                {pill}
              </button>
            ))}
          </div>
        ...
        <div className="max-h-[calc(100%-146px)] overflow-y-auto">
              ...
              <button key={food.id} type="button" onClick={() => toggleFood(food.id)} className="grid w-full grid-cols-[40px_minmax(0,1fr)_280px_58px] items-center border-b border-slate-100 px-3 py-2 text-left hover:bg-slate-50 data-[selected=true]:bg-blue-50/60" data-selected={selected}>
                ...
                <span className="flex min-w-0 items-center gap-2">
                  ...
                  <span className="min-w-0">
                    ...
                    <span className="text-[11px] text-slate-500"><span className="rounded bg-amber-50 px-1 text-amber-700">{food.category}</span> - {food.serving}</span>
                  </span>
                </span>
                <span className="flex gap-2 text-[11px] text-slate-600">
                  <span className="rounded border border-slate-200 px-2 py-1">{food.calories ?? "-"} kcal</span>
                  <span className="rounded border border-slate-200 px-2 py-1">{food.carbs ?? "-"}</span>
                  <span className="rounded border border-slate-200 px-2 py-1">{food.protein ?? "-"}</span>
                </span>
      ...
      <div className="min-h-0 space-y-3 overflow-y-auto pr-1">
            ...
            {selectedFoods.map((food) => (
              <div key={food.id} className="flex items-center gap-2">
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="text-[11px] text-slate-400">{food.category} - {food.serving}</span>
                </span>
        ...
        <InputPreviewPanel title="Nutrition snapshot">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ["Calories", `${focusedFood.calories ?? "-"} kcal`],
              ["Carbs", focusedFood.carbs ?? "-"],
              ["Protein", focusedFood.protein ?? "-"],
              ["Fiber", focusedFood.fiber ?? "-"],
            ].map(([label, value]) => (

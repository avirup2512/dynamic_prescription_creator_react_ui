...
function SideBar({ routeArray, isCollapsed, onToggleCollapse }: any) {
    ...
    return (
        <>
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 flex h-screen shrink-0 flex-col border-r border-border bg-card px-4 py-5 shadow-xl transition-all duration-300 md:static md:h-auto md:shadow-none ${isCollapsed ? '-translate-x-full md:w-20 md:translate-x-0' : 'w-64 translate-x-0 md:w-64'}`}>
                        ...
                        routeArray && routeArray.length > 0 && routeArray.map((e: any, i: any) => {
                            ...
                            if (hasChildren) {
                                return (
                                    <div key={i}>
                                        <button
                                            className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm font-medium transition hover:bg-muted hover:text-foreground ${isChildActive
                                        ...
                                        {isOpen && !isCollapsed ? (
                                            <div className="mt-1 space-y-1 border-l border-border pl-3">
                                                {e.children.map((child: any, childIndex: any) => (
                                                    <NavLink
                                                        className={({ isActive }) =>
                                                            `block rounded-lg px-3 py-2 text-sm font-medium transition hover:bg-muted hover:text-foreground ${isActive
                                                                ? 'bg-muted text-foreground'
                                                                : 'text-muted-foreground'
                                                            }`
                                                        }
